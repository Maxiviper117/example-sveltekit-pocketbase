...
export const pocketbaseHandle: Handle = async ({ event, resolve }) => {
	...
	event.locals.pbUser = new PocketBase(POCKETBASE_URL);
	...
	event.locals.pbUser.authStore.loadFromCookie(event.request.headers.get('cookie') || '');

	try {
		// If a session exists, try to refresh it; on failure clear the auth store
		if (event.locals.pbUser.authStore.isValid) {
			await event.locals.pbUser.collection('users').authRefresh();
		}
		// eslint-disable-next-line @typescript-eslint/no-unused-vars
	} catch (_) {
		// clear the auth store on failed refresh
		event.locals.pbUser.authStore.clear();
	}
	...
	const pb_auth = event.locals.pbUser.authStore.exportToCookie(); // if logging in, this will be populated with the auth token and user data
